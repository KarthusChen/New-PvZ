...
public:
	...
	void on_draw(const Camera& camera)
	{
		...
		putimage_alpha(camera, pos_img_hills.x, pos_img_sky.y, &img_hills);
		...
		if (is_debug)
		{
			...
			outtextxy(15, 15, _T("Debugging Mode is On, Press Q to Turn Off."));
		}
	...
	private:
		const float speed_winner_bar = 3.0f; // Speed of the winner bar sliding in
		const float speed_winner_text = 1.5f; // Speed of the winner text sliding in
